...
DELIVERY_STATUS="$ROOT_DIR/docs/agent-guidance/delivery-status.md"
[[ -f "$DELIVERY_STATUS" ]] || DELIVERY_STATUS="$ROOT_DIR/AGENTS.md"
...
for file in "$ROOT_README" "$MOBILE_README" "$PROTO_README" "$RELEASE_README" "$TEST_SCENARIOS" "$ROOT_AGENTS" "$DELIVERY_STATUS"; do
...
require_pattern "$ROOT_AGENTS" 'Room[[:space:]]+(is[[:space:]]+at[[:space:]]+)?version[[:space:]]+12' 'root AGENTS.md must name Room version 12'
require_literal "$ROOT_AGENTS" 'Migration(12,13)' 'root AGENTS.md must require the next Room Migration(12,13)'
require_pattern "$ROOT_AGENTS" 'schemas?[^0-9]*13\.json' 'root AGENTS.md must require the next committed Room schema 13.json'
...
require_paragraph_pattern "$DELIVERY_STATUS" 'tasks[[:space:]]+1-9.*implementation and host automation.*complete' 'root AGENTS.md must record direct-LAN Tasks 1-9 source and host completion'
require_literal "$DELIVERY_STATUS" 'pending physical two-phone run' 'root AGENTS.md must preserve pending physical two-phone direct-LAN evidence'
reject_pattern "$DELIVERY_STATUS" 'Tasks[[:space:]]+1-4[[:space:]]+landed;[[:space:]]+5-9[[:space:]]+open|complete(d)?[[:space:]]+physical[[:space:]]+two-phone[[:space:]]+run' 'root AGENTS.md must not reopen direct-LAN tasks or complete pending hardware evidence'
require_literal "$DELIVERY_STATUS" 'tracks Plans 001-030' 'root AGENTS.md must name the advisor ledger through Plan 030'
reject_pattern "$DELIVERY_STATUS" 'tracks[[:space:]]+the[[:space:]]+audit-driven[[:space:]]+plans[[:space:]]+001-010' 'root AGENTS.md must not retain the stale advisor ledger range'
require_paragraph_pattern "$DELIVERY_STATUS" 'plan[[:space:]]+004.*externally blocked.*eas.*(project|signing|token|certificate|attestation)' 'root AGENTS.md must preserve Plan 004 protected EAS block'
require_paragraph_pattern "$DELIVERY_STATUS" 'plan[[:space:]]+015.*source is complete.*phy-call-01.*physical proof is deferred' 'root AGENTS.md must preserve Plan 015 deferred physical-call proof'
require_literal "$DELIVERY_STATUS" 'Local APKs are QA artifacts, not protected release candidates.' 'root AGENTS.md must preserve local APK release status'
reject_pattern "$DELIVERY_STATUS" 'Plan[[:space:]]+004[[:space:]]+is[[:space:]]+complete' 'root AGENTS.md must not complete protected EAS work'
