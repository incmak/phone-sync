...
Current state: **the relay and Android both implement v1 + v2**. Android advertises `[2,1]` with `relay.hello`, sends durable v2 envelopes with `relay.put`, authenticates inner events, stores inbound/outbound state in Room version 9 under a route-neutral custody column (`custodyAcceptedAt`/`custodyRoute`), materializes desired notification/call state, and emits peer receipts. The deprecated DataStore replay guard and legacy outbound queue remain only for v1 compatibility/migration; do not route new reliable-delivery work through them.
...
- **Room is at version 9.** New entity → version 10 + an explicit `Migration(9,10)` registered in `NotificationDb.addMigrations(...)`, plus a committed `schemas/.../10.json`. Never `fallbackToDestructiveMigration()`; it wipes paired state.
