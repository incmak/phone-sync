...
e2e-bluetooth-route:
	@test -n "$(E2E_DEVICE_A)" -a -n "$(E2E_DEVICE_B)" -a "$(E2E_DEVICE_A)" != "$(E2E_DEVICE_B)" || { echo "two explicit distinct E2E_DEVICE_A/E2E_DEVICE_B serials are required" >&2; exit 2; }
	@test -n "$(E2E_BLUETOOTH_EVIDENCE_DIR)" || { echo "E2E_BLUETOOTH_EVIDENCE_DIR is required" >&2; exit 2; }
	cd e2e && go run ./cmd/twinotify-e2e -scenario bluetooth-direct-route -serial-a "$(E2E_DEVICE_A)" -serial-b "$(E2E_DEVICE_B)" -scenario-evidence-dir "$(abspath $(E2E_BLUETOOTH_EVIDENCE_DIR))"
