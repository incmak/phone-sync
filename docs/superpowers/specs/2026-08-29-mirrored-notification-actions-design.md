...
- `EnvelopeAuthenticator` allows +5 min past `expires_at`, which is wider than the product window. The origin therefore applies its own action freshness check before executing anything: an `invoked_at` up to 30 seconds ahead of the origin clock is treated as age zero, a larger future difference is rejected, and a timestamp at or behind the origin clock must satisfy `now ≤ invoked_at + 120_000`. The bounded future-skew allowance covers ordinary handset drift without adopting the transport's wider grace; it can extend the mirror-wall-clock interval by at most 30 seconds, while anything already more than 2 minutes old on the origin clock still fails closed.
...
- **Freshness:** origin-side 2-minute age window with at most 30 seconds of bounded future-clock tolerance > inner `expires_at` backstop > mirror-side local expiry. Three independent layers; any one refusing is terminal. Future skew beyond 30 seconds and age beyond 2 minutes both fail closed.
