...
- [x] Reproduce and correct asymmetric relay-to-LAN promotion.
  ...
  - The signed APK was installed in place on the previously relay-side phone with pairing/data preserved. It first reported `Via relay`, established the private-LAN candidate, and then automatically changed to `Direct on Wi-Fi` / `Reachable now` after the configured 30-second stability window. The peer remained on the immediately preceding signed build because its wireless-debugging endpoint dropped during the update attempt, so same-build two-phone acceptance remains pending.
...
- [x] If ADB, relay deployment, or a named physical state is unavailable, leave that checkbox pending and state the exact unverified condition.
  ...
  - During the 2026-09-01 continuation, only one phone advertised a wireless-debugging endpoint. The second phone remained visible to Twinotify's LAN discovery but was absent from `adb mdns services`, so the final signed APK could not be installed or inspected there. No network scan, credential bypass, pairing reset, uninstall, or data clear was attempted. Final-build two-phone promotion, bidirectional delivery, Wi-Fi loss/restore, restart, stale-presence, and screenshot rows remain pending until that phone re-enables wireless debugging.
