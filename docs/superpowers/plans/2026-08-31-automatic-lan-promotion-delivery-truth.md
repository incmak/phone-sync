...
- [x] Reproduce and correct a LAN-deadline failure that stopped relay fallback.
  - On 2026-09-01, a direct-first attempt with the peer unavailable logged `lan_open_failed` and remained `Reconnecting` beyond the coordinator backoff window. Changing the preference back to relay-first immediately restored relay, isolating the fault to the failed-LAN return path rather than relay admission.
  - The whole-attempt `withTimeout` raised `TimeoutCancellationException`; `TransportCoordinator` correctly preserves cancellation identity, so it interpreted that internal deadline as a request to stop instead of as an ordinary failed route. A RED connector test captured the mismatch. `DirectLanConnector` now converts only its own deadline to the existing typed `lan_connection_timeout` failure while genuine external cancellation remains unchanged.
  - The focused regression passed, followed by the complete core JVM/lint gate (`169` tasks). A signed release APK was produced with SHA-256 `4fd0ed71f08ebf3bdc5c309d716c579eaeb9a67a47527c4b8a8381d9716bbf84`; its signer SHA-256 remained `a94183d1a8471e700c0aa9542a40ffd49b3a07c84e40a46daf60deca39b77983`.
  - The APK was installed through Android's data-preserving update path on the reachable phone. With direct Wi-Fi still preferred and the peer unavailable, the phone logged `lan_connection_timeout`, connected to relay without another setting change, and showed `Via relay` in five consecutive samples. This satisfies the single-phone failed-LAN-to-relay regression, not the remaining two-phone transitions.
- [x] Reproduce and correct asymmetric relay-to-LAN promotion.
  - With both phones unlocked on the same Wi-Fi, the direct-first phone reported direct delivery while the relay-side phone retained both an authenticated private-LAN socket and its relay route. Refreshing the foreground UI did not change the mismatch, ruling out a stale React snapshot.
  - `LanRoute.open()` launched its long-lived authenticated worker as a child of the coordinator's temporary promotion `coroutineScope`. That scope therefore could not return the candidate to the coordinator, so relay was never closed and LAN was never granted on that phone. A RED route-lifecycle test reproduced the blocked scope. The returned `LanRouteSession` now owns the worker, and cancellation-resistant close still joins its finalizer before replacement.
  - The focused LAN/coordinator suites and the complete Android gate (`918` tasks), TypeScript check, lint, Expo Doctor, and signed release build passed. The APK SHA-256 is `18b5dadbee673c8f62b2831e24c100578fbaf97d5035a838ad7ca8c6cf8fd359`; the signer SHA-256 remains `a94183d1a8471e700c0aa9542a40ffd49b3a07c84e40a46daf60deca39b77983`.
  - The signed APK was installed in place on the previously relay-side phone with pairing/data preserved. It first reported `Via relay`, established the private-LAN candidate, and then automatically changed to `Direct on Wi-Fi` / `Reachable now` after the configured 30-second stability window.
  - The peer's wireless-debugging endpoint later returned, allowing the same signed APK to be installed through the data-preserving path there as well. Fresh on-device hashes matched the release artifact on both phones. After mirroring resumed, the updated peer reported direct immediately; the other phone carried over relay while the peer was paused and automatically returned to `Direct on Wi-Fi` / `Reachable now` after the full stability window. This closes the same-build asymmetric-promotion regression, but not the separate Wi-Fi-loss/restore or bidirectional-delivery rows below.
- [x] Record partial same-Wi-Fi and notification evidence without promoting it to final-build acceptance.
  - Before the final deadline correction, both unlocked phones on the same Wi-Fi automatically reached authenticated direct mode and remained there for five consecutive samples. Sanitized synthetic notifications were delivered once in each direction over direct Wi-Fi with the expected peer marker. The home mirrored count nevertheless remained zero; that product defect is tracked as PB-009.
  - A relay store-and-forward observation also retained custody without reposting, but receiver recovery required reopening the app and changing a preference after force-stop. That lifecycle defect is tracked as PB-008.
...
- [x] If ADB, relay deployment, or a named physical state is unavailable, leave that checkbox pending and state the exact unverified condition.
  - Both phones remained attached, but the MI 11X entered its secure keyguard during the signed release build. Android correctly rejected a shell `BOOT_COMPLETED` broadcast and the non-exported sync service cannot be started through ADB. No credential or lock-screen bypass was attempted. Those checks remained pending at that point; the operator later unlocked the phone and the same-build promotion result above completed.
  - During the 2026-09-01 continuation, one phone temporarily stopped advertising wireless debugging while remaining visible to Twinotify's LAN discovery. No network scan, credential bypass, pairing reset, uninstall, or data clear was attempted. The endpoint later returned and the same-build promotion row above completed; bidirectional delivery, Wi-Fi loss/restore, restart, stale-presence, and screenshot rows remain pending.
...
- [x] Compare every acceptance criterion in `docs/superpowers/specs/2026-08-31-automatic-lan-promotion-delivery-truth-design.md` against code and artifacts.
  - Criteria 1-2 have protocol, bootstrap, lifecycle, coordinator, host-scenario, and same-build same-Wi-Fi convergence coverage; fresh binding creation remains pending.
  - Criteria 3-4 are covered by single-drainer route-generation evidence, deterministic coordinator promotion/fallback tests, and bounded physical relay-to-LAN promotion; physical Wi-Fi-loss fallback remains pending.
  ...
  - Criterion 12 now includes a sanitized two-phone same-build promotion result; it does not substitute for the still-pending delivery, fallback, restart, stale-presence, or screenshot scenarios.
