...
- [ ] 4. Implement fail-closed direct update, peer-dismiss, call, and snapshot/receipt scenarios
  ...
  - Pattern:  `e2e/internal/scenario/route_evidence.go` - closed-world retained-evidence allowlist; add only the bounded `snapshot_commit_count` observation key.
  - Test:     `e2e/internal/scenario/route_evidence_test.go` - prove the new counter is retained while unknown and out-of-bound observation fields still fail closed.
  ...
  - API/Type: `mobile/modules/twinotify-core/android/src/main/java/co/twinotify/core/service/InboundDispatcher.kt` - record snapshot application only from the successful `SnapshotConvergence.Committed` branch, never from mere authenticated receipt of `state.snapshot.end`.
  - Test:     `mobile/modules/twinotify-core/android/src/test/java/co/twinotify/core/service/InboundDispatcherControlTest.kt` - RED/GREEN proof that commit increments once while mismatch/rejection/exception does not.
  ...
  - [ ] Focused dispatcher/provider and Go parser tests prove `snapshot_commit_count` increments only after a committed Room snapshot, is bounded/content-free, and is required independently from begin/end counters and route custody.
