# Twinotify — Exhaustive Screen + UI Inventory

> **Product:** Twinotify — end-to-end-encrypted notification mirroring between two Android phones (with a Tauri desktop receiver in later phases).
> **Name meaning:** "twin" (paired devices) + "notify" — your notifications, twinned across devices.
> **Namespace:** `com.twinotify.app` (Android), `twinotify.com` / `twinotify.app` / `twinotify.io` (domains).
>
...
>
> **Wordmark/logo guidance for the designer:** the name twins on itself. "Twin" + "otify" — consider a monogram that echoes two halves (mirror-symmetric, two dots, doubled "i"s, linked circles). Color palette should feel secure + calm (the product handles private messages) but not cold/clinical. Avoid generic "sync arrows" — this is not a cloud-backup app, it's a device-pair utility.
...
Mirrored notifications posted BY Twinotify itself. Follow the spec's Phase 1 policy: single `mirrored_notifications` channel, `IMPORTANCE_DEFAULT`, `VISIBILITY_PRIVATE` by default.
...
| MIR-05 | Persistent FG notification              | M-Notif  | Required for the foreground SyncService         | Status line ("Twinotify active — LAN connected"), quick actions (pause / open app)                     |
...
## 9 · Notifications from Twinotify itself (not mirrors)
